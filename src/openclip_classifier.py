...
Self-contained per-model module (mirrors the SigLIP pattern). Reuses the
shared classifier builder and splitter from dinov3_classifier, but keeps its
own CV loop so it can save per-fold predictions for error analysis.
"""
...
def cross_validate_openclip_classifier(
    ...
    *,
        ...
        fold_rows.append(
            {
                ...
                "strategy": f"openclip_frozen_embeddings_{classifier}",
    ...
    summary = summarize_openclip_folds(folds)
    return summary, folds, predictions_df


def summarize_openclip_folds(fold_results: pd.DataFrame) -> pd.DataFrame:
    """Summarize per-fold OpenCLIP classifier metrics."""
