...
SigLIP. Single source of truth for CV logic, per-model entry point preserved.
"""
...
def _relabel_strategy(frame: pd.DataFrame) -> pd.DataFrame:
    """Rewrite the dinov3_* strategy label to siglip_* in a result frame."""
...
def cross_validate_siglip_classifier(
    ...
    *,
    ...
    relabels the strategy column to mark these as SigLIP results.
...
def summarize_siglip_folds(
    fold_results: pd.DataFrame,
    *,
    predictions: pd.DataFrame | None = None,
    per_asset_type: bool = False,
) -> pd.DataFrame:
    """Summarize SigLIP fold metrics (delegates to the shared summarizer)."""
