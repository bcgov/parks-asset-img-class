---
title: ""

bibliography: references.bib
jupyter: bcparks_capstone
author:
  - name: Sarah Gauthier
    affiliations:
      - name: Master of Data Science, University of British Columbia
  - name: Nour Shawky
    affiliations:
      - name: Master of Data Science, University of British Columbia
  - name: Ssemakula Peter Wasswa
    affiliations:
      - name: Master of Data Science, University of British Columbia
  - name: Mehmet Imga
    affiliations:
      - name: Master of Data Science, University of British Columbia

format:
  html:
    toc: true
    toc-depth: 2
    number-sections: true
    css: report-styles.css
  pdf:
    pdf-engine: lualatex
    toc: false
    toc-depth: 2
    number-sections: true
    colorlinks: true
    fontsize: 10pt
    geometry:
      - margin=0.85in
    header-includes:
      - \linespread{0.98}
---

```{=html}
<section class="report-header">
  <h1>Final Report</h1>
  <h2>Image Analysis of Park Infrastructure</h2>

  <dl>
    <dt>Date:</dt>
    <dd>2026-06-24</dd>

    <dt>Prepared for:</dt>
    <dd>BC Parks &middot; UBC Master of Data Science Capstone</dd>

    <dt>Prepared by:</dt>
    <dd>
      Sarah Gauthier<br>
      Nour Shawky<br>
      Ssemakula Peter Wasswa<br>
      Mehmet Imga
    </dd>
  </dl>
</section>
```

```{=latex}
\begin{center}
{\Huge \textbf{Final Report}\par}
\vspace{0.35cm}
{\Large \textbf{Image Analysis of Park Infrastructure}\par}
\end{center}

\vspace{0.45cm}

\begin{center}
\begin{tabular}{r@{\hspace{0.7cm}}l}
\textbf{Date:} & 2026-06-25 \\
\textbf{Prepared for:} & BC Parks \\
\textbf{Prepared by:} & Sarah Gauthier \\
                     & Nour Shawky \\
                     & Ssemakula Peter Wasswa \\
                     & Mehmet Imga \\
\end{tabular}
\end{center}

\vspace{0.55cm}

\newpage
\tableofcontents
\newpage
```

# Executive summary

# Introduction 

## Background and problem statement

## Refined objectives

# Data

## Data sources and collection

## Size and characteristics

## Data quality, limitations, and preprocessing

# Data science methods

## Approaches considered

## VLM Approach

## Embedding-based classification

# Data product and results

## The Data Product

## Results

# Conclusions and recommendations

BC Parks maintains a large inventory of infrastructure assets that are not just limited to stairs, trail bridges, boardwalks and viewing platforms, and for which many have incomplete labels. Verifying these attributes in the field is costly and slow, but is necessary to make decisions about safety, inspection scheduling, and resource allocation. The original problem was therefore whether these missing attributes could be predicted directly from existing asset photographs, reducing the manual effort required to keep records complete and current and shiftng to a more automated attribute-labelling-pipeline, helping allocate manual resources elsewhere.

Our product addresses this by predicting attribute labels given a set of asset images alongside corresponding confidence scores that may signal the need for manual intervention, which helps direct limited field-verification effort toward the predictions that most warrant a human check, and the more confident predictions to be automated. 

The extent to which the product meets BC Parks' needs varies by attribute:
- Categorical attributes that are visually distinctive such as [decking material, structure material, and the presence of railings], both VLM and Embedding models (DINOv3) provide labels that are dependable enough to reduce manual labelling meaningfully. 
- Measurement-based attributes such as length, width, fall height and number of steps are considerably harder, because estimating a physical dimension or bin from a photograph without a scale reference is inherently difficult, and because each asset type defines these bins differently. 
- Here the model will help but more manual review will be required, rather than fully relying on automated labelling from our pipeline. This signals to BCParks that our model's performance is not as trustworthy for these attributes (refer to weighted f1 scores showing lower f1 for numeric attributes compared to categorical/material attributes). 
- DINOv3 performs consistently well across all attributes (with exception of length & number of steps). We recommend using it as the default model
- For length and number of steps, a combined sub-pipeline (where length is predicted and verified by both models) is recommended for optimal labelling accuracy.***subject to change 

Gaps:
- less trustworthy performance on uncommon and ambigious attribute classes 
- predicting measurement based attributes reliably 

Key lesson learned:
- Difficulty of an attribute depends less on the choice of model than on whether the attribute is visually determinable at all
- visually distinctive attributes were learned well by multiple approaches whereas measurement attributes weren't.
- shifted our framework from "which model is best" toward "which attributes can be automated reliably, and which "need a human in the loop."

Future work:
- Fine tuning/ few shot learning with VLM and embedding models 
- Using open source data to expand our training and validation sets (obtain more labelled data)
- Depth estimation models (for numerical attributes)
- Obtain scale & size reference for materials used to build infrastructure from BCParks (to feed as information to our models)

# References