...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory where classifier metric CSVs are written. Defaults to the classifier subfolder.",
    )
    parser.add_argument(
        "--predictions-dir",
        type=Path,
        default=None,
        help="Directory where prediction CSVs are written. Defaults to the classifier subfolder.",
    )
    parser.add_argument(
        "--model",
        default="ViT-B-16",
...
def main() -> int:
    ...
    for target in targets:
        ...
        if args.output_dir is not None:
            classifier_command.extend(["--output-dir", str(args.output_dir)])
        if args.predictions_dir is not None:
            classifier_command.extend(["--predictions-dir", str(args.predictions_dir)])
